  ...
  $(elm).find(hoverClass).removeClass(hoverClass).addClass(focusedClass);
        ...
        if (accOn === true) {
            ...
            switch (key) {
				case 77:  //M
                    if (!popupOpen) {
                        const totalLi = $('#moleculesList li').length;
                        const completedLi = $('#moleculesList .checkImg.visible').length;
                        if (totalLi === completedLi) {
                            $("#moleculesList li").removeClass('active');
                            $("#moleculesList li:first-child").addClass('active');
                            $("#moleculesList li:first-child a").trigger("click");
                            $("#sidePanel_m").addClass("currentSection");
                            $('#sidePanel_procedure').removeClass("currentSection");
                            $('.rightPanel-text p').removeClass('currentSection');
                            $('.footer-text p').removeClass('currentSection');
                            return;
                        }

						if ($('.rightPanel').find('.rightsideBtnHover').length > 0 && $('.rightPanel-text').find('.currentSection').length > 0) {
                            if ($('.rightsideBtnHover').hasClass('chooseAgain')) {
                                $('.rightsideBtnHover').removeClass('chooseAgain');
                                $('.rightsideBtnHover').html('');
                            }

                            $('.rightPanel').find('.rightsideBtnHover').removeClass('rightsideBtnHover');
                            $('.rightPanel-text').find('.currentSection').removeClass('currentSection');
                        }
						...
						$('.drag-container.visibleDragContainer').find('.focused').removeClass('focused');
                ...
                case 80:  //P
                    if ($('#sidePanel_procedure').is(':visible') && !popupOpen) {
						...
						$('#sidePanel_procedure').addClass("currentSection");
						$("#sidePanel_m").removeClass("currentSection");
						$('.rightPanel-text p').removeClass('currentSection');
						$('.footer-text p').removeClass('currentSection');
						$('.footer ul li').removeClass('focused');
			        } 
                    break;
                case 37:  //LEFT ARROW
                    if (popupOpen) {
			   ...
			   case 66: //B - Bonds selection
                        if (!popupOpen) {
                          if (($('#sidePanel_procedure').hasClass('ui-state-active') && $(".molecules-block").find(".active").length < 1) ||  $('.rightPanel').find('.enabled').length > 0) {

							  return;
							}
							if($(".molecules-block").find(".active").length < 1){
								return;
							}
							$('.drag-container.visibleDragContainer').removeClass('active');
							...
							$('.rightPanel-text p').addClass('currentSection');
							$('.footer-text p').removeClass('currentSection');
							$("#sidePanel_m").removeClass("currentSection");
							$("#sidePanel_procedure").removeClass("currentSection");
			  ...
			  case 38:  //UP ARROW
                    if (!popupOpen) {
                            ...
                            const completedLi = $('#moleculesList .checkImg.visible').length;
                            if (totalLi === completedLi) {
                                const prevDiv = $('#moleculesList .active').prev();
                                $('#moleculesList li').removeClass('active');
                                if($(prevDiv).length < 1) {
                                    $('#moleculesList li:last-child').addClass('active');
                                    $('#moleculesList li:last-child a').trigger('click');
                                } else {
                                    $(prevDiv).addClass('active');
                                    $(prevDiv).find('a').trigger('click');
                                }
                                return;
                            }
                            ...
                            if ($("#moleculesList").find(".focused").length < 1 && $(".rightPanel").find(".enabled").length < 1 && !$('#sidePanel_m').hasClass('currentSection')) {
							...
							if ($(".rightPanel").find(".rightsideBtnHover").length > 0 || ($(".rightPanel").find(".enabled").length > 0 && $('#moleculesList').find('.focused').length < 1)) {
							  return;
							}
							let selector = "";
							if ($("#moleculesList").find(".focused").length > 0) {
           ...
           case 40:  //DOWN ARROW
                        event.preventDefault();
						  if (!popupOpen) {
                            ...
                            const completedLi = $('#moleculesList .checkImg.visible').length;
                            if (totalLi === completedLi) {
                                const nextDiv = $('#moleculesList .active').next();
                                $('#moleculesList li').removeClass('active');
                                if($(nextDiv).length < 1) {
                                    $('#moleculesList li:first-child').addClass('active');
                                    $('#moleculesList li:first-child a').trigger('click');
							  ...
							  if (atom.length > 0) {
								...
								$(spans).each((index, sp) => {
								  if ($(sp).attr("data-id").indexOf($(currentElm).attr("data_id")) > -1 && $(sp).children().length < 1) {
									...
									$(currentElm).html("").addClass("dragged").removeClass("focused");
									  $(currentElm).next().addClass("focused");
									return false;
								  }
								}); 
								...
								if (counter === $(spans).length) {
									...
									if($('#correctMessage').hasClass('visible')){
									$('#sidePanel_procedure').removeClass("currentSection");
									$("#sidePanel_m").removeClass("currentSection");
									$('.footer-text p').removeClass('currentSection');
									$('.rightPanel-text p').removeClass('currentSection');
									 $(currentElm).next().removeClass("focused");
									 }
				...
				case 65:// 	A - select Atoms
					if (!popupOpen) {
						const mainContainer = $(".footer .drag-container.visibleDragContainer");
						//if (($(".molecules-block").hasClass("activePanel") && $(".molecules-block").find(".focused").length > 0) || $(".rightPanel").hasClass("disabled") || $('.rightPanel').find('.enabled').length < 1)  {
						if ($(".rightPanel").hasClass("disabled") || $('.rightPanel').find('.enabled').length < 1)  {
						  return;
						}
						$('.molecules-block').removeClass('activePanel');
						$("#moleculesList").find(".focused").removeClass("focused");
						...
						const firstUl = $(mainContainer).find("ul:nth-child(1)");
						if ($(mainContainer).find(".focused").length < 1) {
						  	var index = giveNextActiveIndex($(firstUl).find("li")) + 1;
							if(!$('#correctMessage').hasClass('visible')){
						  	$(firstUl).find("li:nth-child(" + index + ")").addClass("focused"); 
							$('.footer-text p').addClass('currentSection');}
							$('.rightPanel-text p').removeClass('currentSection');
							$('#sidePanel_procedure').removeClass("currentSection");
							$("#sidePanel_m").removeClass("currentSection");
						 	return;
